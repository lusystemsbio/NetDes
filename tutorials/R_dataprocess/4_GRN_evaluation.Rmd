---
title: "Racipe_simulation and GRN chosing"
author: "YUKAI YOU"
date: '2024-10-11'
...
After obtaining the GRN results, you can use RACIPE to model the dynamics of each GRN. First, input all GRNs into the list `Networklist_noadd`. This step prepares your networks for further analysis and comparison with the scRNA-seq data.

You still need source the function at the beginning.
...
**Step 2: Mapping GRN Dynamics to scRNA-seq Trajectories**

Once the GRNs are modeled, you can compare the network dynamics to the smoothed gene expression trajectories from scRNA-seq data. In this step, map the results will shows in the following metrics:
...
-   `NetDesnoadd_entropy`: The entropy we defined for each GRN, representing the diversity of its network dynamics.
...
Normally based on percentage_NetDesnoadd_3D you can pick one network with highest mapping percentage.

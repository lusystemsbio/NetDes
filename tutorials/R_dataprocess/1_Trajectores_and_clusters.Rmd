---
title: "1.Trajectories_infer_cluster"
author: "YUKAI YOU"
date: '2024-10-09'
output:
...
#### **Step 1: Load the scRNA-seq Data Start by loading the necessary scRNA-seq data.** 

The raw data should be processed with SCTransform for pseudotime analysis.
...
Raw data process with SCTransform for analyzing pseudotime.
...
#### Step 2: I dentify HVGs

Occasionally, gene names containing "-" may be altered during processing. Make sure to modify them to maintain consistency with the `pbmc` dataset results. Additionally, calculate the top 500 and top 5000 highly variable genes (HVGs) using the **scran** package.
...
Once the top 500 HVGs are identified, use the processed scRNA-seq data to compute pseudotime.
...
In this case, certain pseudotime ranges have sparse cell representation. For better fitting, some cells may need to be removed, and pseudotime should be rescaled accordingly.
...
#### Step 5: Smooth Gene Expression Trajectories

Use pseudotimeDE to fit the top 5000 HVGs. If **BiocParallel** is not utilized, adjust the cores setting to `mc.cores = 1` if necessary.
...
Obtain smoothed gene expression trajectories over time.
...
Cluster the gene expression trajectories into 7 distinct clusters. Finally, visualize the clustering results.

...
#### Step 1: Load the NetAct Database and Cluster Results 

Begin by loading the NetAct database and the cluster results.
For the database, retain only the transcription factors (TFs) that have more than 5 target genes.
In the cluster results, remove Cluster 7, as this cluster consists primarily of noise genes.
...
#### Step 2: Apply Fisher's Exact Test 

Perform a Fisher's exact test to identify genes with a q-value smaller than 0.1. We only consider the TFs within the 6 clusters.
...
#### Step 3: Analyze and Filter TFs

For each TF, check if their target gene lists overlap significantly with the target lists of other TFs.
If a pair of TFs has a high overlap in target genes, select the TF with the higher standard deviation of expression.
Delete the other TF from the analysis to reduce redundancy.
